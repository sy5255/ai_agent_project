...
- [Part 3. 전체 기능 명세 (F-01 ~ F-93)](#part-3-전체-기능-명세)
...
Part 3의 기능들은 모두 `F-xx` 번호를 가진다. (F-01~F-58은 제품 기능, F-59~F-79는 모델 API 통합·배포 프로파일, F-80~F-93은 실제 계측 표시 샘플 분석에서 도출된 기하·UI 요구사항이다.) Part 6의 마일스톤과 Part 7의 KPI가 이 번호를 참조하므로, 개발 착수 시 이 번호를 그대로 이슈 트래커의 Epic/Story 키로 사용하면 된다.
...
```
 y= 14  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  기준선 A (전폭)
                        ┃ x=158
                        ┃  ← 빨강 수직선 = **오프셋 가이드** (기준선 B를 A로부터 66px 아래로 정의)
 y= 80  ━━━━┳━━━━┳━━━━┳━┻━━┳━━━━┳━━━━┳━━━━━━━━  기준선 B + 초록 6구간 (기둥 CD)
             ▔▔   ▔▔   ▔▔   ▔▔   ▔▔   ▔▔
 y=137  ━━━━┳━━━━┳━━━━┳━━━━┳━━━━┳━━━━┳━━━━━━━━  기준선 C + 초록 6구간 (같은 기둥, 다른 높이)
 y=142  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  기준선 D
```
...
>
> 이것이 이번 분석에서 가장 값진 발견이다. 오프셋 가이드를 인식하지 못하면 시스템은 y=80을 절대 좌표로 굳혀 일반화에 실패하지만, 인식하면 **질문조차 필요 없이** 올바른 상대 정의를 얻는다. 반대로 오프셋 가이드가 **없는** 기준선에 대해서는 반드시 물어야 한다(F-82).
...
- **인수 조건** 샘플 A를 임포트했을 때 x=158 빨간 선이 **`offset_guide(datum_A → datum_B, 66 px)`** 로 분류되고, 그 결과 기준선 B의 위치가 **절대 y=80이 아니라 "A로부터 66 px 아래"** 로 MIR에 기록된다.
...
- **질문은 오프셋 가이드가 없는 기준선에만 던진다.** 근거 없이 떠 있는 기준선이 곧 일반화 실패 후보이므로, 표적 질문의 대상을 여기로 좁힌다.
...
```jsonc
{
  ...
  "entities": [
    ...
    { "id": "offset_A_to_B", "layer": "guide", "kind": "offset_guide",
      "from": "entity:datum_A", "to": "entity:datum_B",
      "length_px": 66.0, "length_nm": 128.9,
      "endpoints_semantic": true,
      "note": "이 획의 길이가 datum_B의 정의다. 출력 측정값은 아니다.",
      "provenance": { "source": "snap", "confidence": 0.96 } },
...
- **★ 샘플 A/B 회귀 통과**: 두 샘플을 임포트했을 때 (a) 샘플 A의 x=158 획이 `offset_guide` 로 인식되어 기준선 B가 **상대 정의**로 기록되고, (b) 샘플 B의 **9 px 기준선이 삭제되지 않으며**, (c) 초록 구간 12개가 2개의 컷라인 × `all_occurrences` 로 압축된다.
